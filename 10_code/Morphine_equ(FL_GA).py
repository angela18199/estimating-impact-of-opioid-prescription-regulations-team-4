...
data2 = pd.read_csv('arcos-ga-statewide-itemized.tsv',delimiter = '\t')
...
drug_data2.isnull().sum() #Drop 114 missing data
drug_data2.dropna(inplace = True)
#Obtain GA drug data with FIPS
GA_FIPS = county[county['State Abbreviation'] == 'GA']
GA_FIPS['GU Name'] = GA_FIPS['GU Name'].str.upper()
GA_FIPS_code = GA_FIPS[['GU Name','County_FIPS']]
GA_drug = pd.merge(GA_FIPS_code,drug_data2,left_on = 'GU Name', right_on = 'BUYER_COUNTY',how = 'right')
GA_drug['GU Name'].unique()
GA_drug['BUYER_COUNTY'].unique()
#Fill the missing FIPS code
GA_drug[GA_drug['BUYER_COUNTY'] == 'RICHMOND'] = GA_drug[GA_drug['BUYER_COUNTY'] == 'RICHMOND'].fillna('13247')
GA_drug[GA_drug['BUYER_COUNTY'] == 'MUSCOGEE'] = GA_drug[GA_drug['BUYER_COUNTY'] == 'MUSCOGEE'].fillna('13217')
GA_drug[GA_drug['BUYER_COUNTY'] == 'CLARKE'] = GA_drug[GA_drug['BUYER_COUNTY'] == 'CLARKE'].fillna('13059')
#drop duplicated columns
GA_drug.drop(columns = 'GU Name',inplace = True)
GA_drug['Year'] = GA_drug['TRANSACTION_DATE'].apply(trans_year)#GET year
GA_drug.drop(columns = 'TRANSACTION_DATE',inplace = True)
GA_drug['Morphine_equ'] = GA_drug['CALC_BASE_WT_IN_GM'] * GA_drug['MME_Conversion_Factor']
GA_drug['State'] = 'GA'
#Groupby County and Year
GA_drug_grouped = GA_drug[['County_FIPS','BUYER_COUNTY','Year','Morphine_equ']]
GA_grouped = GA_drug_grouped.groupby(['Year','County_FIPS','BUYER_COUNTY']).sum()
GA_grouped = GA_grouped.reset_index()
#Take a look at the data
GA_grouped.head()